...
const Team = () => {
  ...
  return (
    <section id="team" className="relative w-full bg-slate-600 py-16 flex items-center justify-center overflow-hidden font-['Courier_New',Courier,monospace]">
      ...
      <button 
        onClick={scrollLeft}
        className="absolute left-0 top-1/2 -translate-y-1/2 z-30 bg-black opacity-80 text-white p-4 hover:opacity-100"
      >
      ...
      <button 
        onClick={scrollRight}
        className="absolute right-0 top-1/2 -translate-y-1/2 z-30 bg-black opacity-80 text-white p-4 hover:opacity-100"
      >
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
